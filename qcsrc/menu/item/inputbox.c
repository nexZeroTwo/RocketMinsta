...
float mouseDragInputBox(entity me, vector pos)
{
	...
	me.cursorPos = draw_TextLengthUpToWidth(me.text, p / me.realFontSize_x, 0, me.fontXYSize);
...
#define CURSOR "_"
	...
	cursorPosInWidths = draw_TextWidth(substring(me.text, 0, me.cursorPos), 0, me.fontXYSize) * me.realFontSize_x;
	totalSizeInWidths = draw_TextWidth(strcat(me.text, CURSOR), 0, me.fontXYSize) * me.realFontSize_x;
	...
	if(me.editColorCodes)
	{
		...
		for(i = 0; i < n; ++i)
		{
			...
			if(ch == "^")
			{
				...
				w = draw_TextWidth(strcat(ch, ch2), 0, me.fontXYSize) * me.realFontSize_x;
				if(ch2 == "^")
				{
					...
					if (component >= 0) // ^xr found
					{
						...
						if (component >= 0) // ^xrg found
						{
							...
							if (component >= 0) // ^xrgb found
							{
								...
								w = draw_TextWidth(substring(me.text, i, 5), 0, me.fontXYSize) * me.realFontSize_x;
							...
							{
								// blue missing
								w = draw_TextWidth(substring(me.text, i, 4), 0, me.fontXYSize) * me.realFontSize_x;
						...
						{
							// green missing
							w = draw_TextWidth(substring(me.text, i, 3), 0, me.fontXYSize) * me.realFontSize_x;
			...
			p += eX * draw_TextWidth(ch, 0, me.fontXYSize) * me.realFontSize_x;
		}
	}
	else
