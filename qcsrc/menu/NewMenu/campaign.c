#ifdef INTERFACE
CLASS(NewMenuCampaignList) EXTENDS(NewMenuListBox)
	METHOD(NewMenuCampaignList, configureNewMenuCampaignList, void(entity))
	ATTRIB(NewMenuCampaignList, rowsPerItem, float, 10)
	METHOD(NewMenuCampaignList, draw, void(entity))
	METHOD(NewMenuCampaignList, drawListBoxItem, void(entity, float, vector, float))
	METHOD(NewMenuCampaignList, clickListBoxItem, void(entity, float, vector))
	METHOD(NewMenuCampaignList, resizeNotify, void(entity, vector, vector, vector, vector))
	METHOD(NewMenuCampaignList, setSelected, void(entity, float))
	METHOD(NewMenuCampaignList, keyDown, float(entity, float, float, float))
	METHOD(NewMenuCampaignList, campaignGo, void(entity, float))
	METHOD(NewMenuCampaignList, destroy, void(entity))

	ATTRIB(NewMenuCampaignList, campaignGlob, float, 0)
	ATTRIB(NewMenuCampaignList, realFontSize, vector, '0 0 0')
	ATTRIB(NewMenuCampaignList, columnPreviewOrigin, float, 0)
	ATTRIB(NewMenuCampaignList, columnPreviewSize, float, 0)
	ATTRIB(NewMenuCampaignList, columnNameOrigin, float, 0)
	ATTRIB(NewMenuCampaignList, columnNameSize, float, 0)
	ATTRIB(NewMenuCampaignList, columnCheckMarkOrigin, float, 0)
	ATTRIB(NewMenuCampaignList, columnCheckMarkSize, float, 0)
	ATTRIB(NewMenuCampaignList, checkMarkOrigin, vector, '0 0 0')
	ATTRIB(NewMenuCampaignList, checkMarkSize, vector, '0 0 0')
	ATTRIB(NewMenuCampaignList, realUpperMargin1, float, 0)
	ATTRIB(NewMenuCampaignList, realUpperMargin2, float, 0)

	ATTRIB(NewMenuCampaignList, lastClickedMap, float, -1)
	ATTRIB(NewMenuCampaignList, lastClickedTime, float, 0)

	ATTRIB(NewMenuCampaignList, origin, vector, '0 0 0')
	ATTRIB(NewMenuCampaignList, itemAbsSize, vector, '0 0 0')
	ATTRIB(NewMenuCampaignList, emptyLineHeight, float, 0.5)

	ATTRIB(NewMenuCampaignList, campaignIndex, float, 0)
	ATTRIB(NewMenuCampaignList, cvarName, string, string_null)
	METHOD(NewMenuCampaignList, loadCvars, void(entity))
	METHOD(NewMenuCampaignList, saveCvars, void(entity))

	ATTRIB(NewMenuCampaignList, buttonNext, entity, NULL)
	ATTRIB(NewMenuCampaignList, buttonPrev, entity, NULL)
	ATTRIB(NewMenuCampaignList, labelTitle, entity, NULL)
ENDCLASS(NewMenuCampaignList)
entity makeNewMenuCampaignList();
...
{
	entity me;
	me = spawnNewMenuCampaignList();
	me.configureNewMenuCampaignList(me);
	return me;
}
void configureNewMenuCampaignListNewMenuCampaignList(entity me)
{
...
void destroyNewMenuCampaignList(entity me)
{
	if(me.campaignGlob >= 0)
		search_end(me.campaignGlob);
}

void loadCvarsNewMenuCampaignList(entity me)
{
	// read campaign cvars
	if(campaign_name)
...
void saveCvarsNewMenuCampaignList(entity me)
{
...
void campaignGoNewMenuCampaignList(entity me, float step)
{
...
void resizeNotifyNewMenuCampaignList(entity me, vector relOrigin, vector relSize, vector absOrigin, vector absSize)
{
...
void clickListBoxItemNewMenuCampaignList(entity me, float i, vector where)
{
...
void drawListBoxItemNewMenuCampaignList(entity me, float i, vector absSize, float isSelected)
{
	string s;
	float p;
	vector theColor;
...
void setSelectedNewMenuCampaignList(entity me, float i)
{
	// prevent too late items from being played
	setSelectedListBox(me, min(i, me.campaignIndex));
}

float keyDownNewMenuCampaignList(entity me, float scan, float ascii, float shift)
{
