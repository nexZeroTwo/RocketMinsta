#ifdef INTERFACE
CLASS(NewMenuPlayerList) EXTENDS(NewMenuListBox)
	ATTRIB(NewMenuPlayerList, rowsPerItem, float, 1)
	METHOD(NewMenuPlayerList, resizeNotify, void(entity, vector, vector, vector, vector))
	METHOD(NewMenuPlayerList, drawListBoxItem, void(entity, float, vector, float))
	ATTRIB(NewMenuPlayerList, realFontSize, vector, '0 0 0')
	ATTRIB(NewMenuPlayerList, columnNameOrigin, float, 0)
	ATTRIB(NewMenuPlayerList, columnNameSize, float, 0)
	ATTRIB(NewMenuPlayerList, columnScoreOrigin, float, 0)
	ATTRIB(NewMenuPlayerList, columnScoreSize, float, 0)
	ATTRIB(NewMenuPlayerList, realUpperMargin, float, 0)
	ATTRIB(NewMenuPlayerList, origin, vector, '0 0 0')
	ATTRIB(NewMenuPlayerList, itemAbsSize, vector, '0 0 0')
	METHOD(NewMenuPlayerList, setPlayerList, void(entity, string))
	METHOD(NewMenuPlayerList, getPlayerList, string(entity, float, float))
	ATTRIB(NewMenuPlayerList, playerList, float, -1)
ENDCLASS(NewMenuPlayerList)
entity makeNewMenuPlayerList();
#endif
...
{
	entity me;
	me = spawnNewMenuPlayerList();
	me.configureNewMenuListBox(me);
	return me;
}

void setPlayerListNewMenuPlayerList(entity me, string plist)
{
	dprint(plist,"------------\n");
...
string getPlayerListNewMenuPlayerList(entity me, float i, float key)
{
	return bufstr_get(me.playerList, i * PLAYERPARM_COUNT + key);
}

void resizeNotifyNewMenuPlayerList(entity me, vector relOrigin, vector relSize, vector absOrigin, vector absSize)
{
...
void drawListBoxItemNewMenuPlayerList(entity me, float i, vector absSize, float isSelected)
{
	string s;
	string score;
	float t;
