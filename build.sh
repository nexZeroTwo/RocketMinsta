...
function make-beastiary
{
	cat << EOF

// Auto-generated by build.sh, GTFO plz

entity monster_spawn(string);

$(for monster in $(list-monsters); do cat << EOB
monster_setup_f monster_${monster}_setup;

entity monster_${monster}_spawn() {
	return monster_spawn_ex("monster_$monster", monster_${monster}_setup);
}
...
monster_setup_f monster_getsetupfunc(string mon) {
	switch(mon) {
$(for monster in $(list-monsters); do echo \
"		case \"$monster\":		return monster_${monster}_setup;"
done)
	}
	return SUB_Remove;
}

entity monster_spawn(string mon) {
	switch(mon) {
$(for monster in $(list-monsters); do echo \
"		case \"$monster\":		return monster_${monster}_spawn();"
done)
	}
...
function buildall
{
	...
	COMMONSUM="$(find "$QCSOURCE/common" -type f | grep -v "fteqcc.log" | grep -v "rm.autogen.qh" | xargs md5sum | md5sum | sed -e 's/ .*//g')"
	MENUSUM="$MENUSUM$COMMONSUM"

    echo "#define RM_BUILD_DATE \"$BUILD_DATE ($2)\"" >  "$QCSOURCE"/common/rm.autogen.qh
    echo "#define RM_BUILD_NAME \"RocketMinsta$1\""   >> "$QCSOURCE"/common/rm.autogen.qh
    echo "#define RM_BUILD_VERSION \"$VERSION\""      >> "$QCSOURCE"/common/rm.autogen.qh
    echo "#define RM_BUILD_MENUSUM \"$MENUSUM\""      >> "$QCSOURCE"/common/rm.autogen.qh
    echo "#define RM_BUILD_SUFFIX \"${1##-}\""        >> "$QCSOURCE"/common/rm.autogen.qh
    echo "#define RM_BUILD_COMMIT \"$COMMIT\""        >> "$QCSOURCE"/common/rm.autogen.qh

	echo "#define RM_SUPPORT_CLIENTPKGS"              >> "$QCSOURCE"/common/rm.autogen.qh
	for i in $BUILT_PKGNAMES; do
		echo "#define RM_SUPPORT_PKG_$i"              >> "$QCSOURCE"/common/rm.autogen.qh
	done
