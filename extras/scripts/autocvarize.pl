...
print "#ifdef RM_AUTOCVARS\n\n";
print "#define CVAR(c) autocvar_##c\n";
print "#define CVAR_STR(c) autocvar_##c\n";
print "#define CVAR_OR(c,d) autocvar_##c\n\n";
...
print "\n#else\n\n";
print "#define CVAR(c) cvar(#c)\n";
print "#define CVAR_STR(c) cvar_string(#c)\n";
print "#define CVAR_OR(c,d) cvar_or(#c, d)\n";
print "#endif\n";
...
for my $f(@files)
{
    ...
    while(<$fh>)
    {
        ...
        s{\b(cvar|cvar_string|cvar_or)\s*\(\s*"([^"]+)"\s*(?:,\s*(.*[^\s]+))?\s*\)}{
